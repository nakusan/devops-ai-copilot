...
运维文档常含页眉页脚；V1 可加规则剔除「第 N 页」等模式。

#### 3.4.4 切块（chunker.py）

**MVP 策略：Recursive Character Text Splitter 思路**

```python
@dataclass
class ChunkConfig:
    chunk_size: int = 512        # 按 token 近似：chars/4 或 tiktoken
    chunk_overlap: int = 64
    separators: list[str] = field(default_factory=lambda: ["\n\n", "\n", "。", " ", ""])

def chunk_text(text: str, cfg: ChunkConfig) -> list[TextChunk]:
    # 按分隔符递归切，保证每块 <= chunk_size，相邻 overlap
    ...
```

| 参数 | MVP | 说明 |
|------|-----|------|
| chunk_size | 512 tokens（约 2000 字符） | 与 Embedding 模型上下文匹配 |
| overlap | 64 tokens | 避免句意被截断 |
| metadata | page 号 | 来自 parser 注释 |

**业界**：

- 固定长度：简单，MVP 够用  
- 语义切块（Semantic Chunker）：V1.1，按 embedding 相似度断句  
- 结构化文档：按标题 `##` 切（Markdown 专用）
