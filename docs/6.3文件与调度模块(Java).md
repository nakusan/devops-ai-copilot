...
| DELETE | `/api/v1/knowledge/documents/{id}` | 204；处理中返回 409 CONFLICT |
| POST | `/api/v1/knowledge/documents/init-upload` | 200 presigned（V1） |
| POST | `/api/v1/knowledge/documents/{id}/confirm` | 202（V1） |

**DELETE 语义（硬删，不可恢复）**

按 `userId` 校验归属，非本人 403、不存在 404。仅允许删除终态（`COMPLETED` / `FAILED`）；
`PENDING` / `PROCESSING` 返回 409，因为此时 Kafka worker 可能正在处理，删行会让它的状态回调撞 404。

执行顺序是先 DB 后 MinIO，且**刻意不加 `@Transactional`**：

1. `DELETE FROM knowledge_documents WHERE id=?` —— `knowledge_chunks` 与 `ingest_jobs`
   的外键均为 `ON DELETE CASCADE`，单条语句已由 PG 原子地清完三张表；
2. 删 MinIO 上的 `knowledge/{id}/original.*` —— 失败只记 `20.minio_orphan` WARN，不影响接口成功。

顺序不能颠倒：残留 MinIO 对象只是存储垃圾，而残留 DB 行会让 RAG 继续命中已删切片、
给出无法溯源的引用。删除后 RAG 立即失效（Python 侧按 `status='COMPLETED'` + `team_id` 只读查 pgvector）。
`deleteById` 返回 0 时按幂等静默返回，覆盖并发重复删除。
...
| DELETE | `/api/v1/analysis/jobs/{id}` | 204；处理中返回 409 CONFLICT |

**DELETE 语义（硬删，不可恢复）**

归属校验与状态守卫同知识库文档。`analysis_jobs` 没有任何指向它的外键，一条 DELETE 即可，
不涉及 CASCADE；但 MinIO 上有**两个**独立对象要清：源文件 `analysis/{id}/source.*`
与结果 `analysis/{id}/result.json`（未跑完时 `resultObjectKey` 为 null，由 `removeQuietly` 内部忽略）。
