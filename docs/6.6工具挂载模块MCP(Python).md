...
> 模块定位：**外部系统能力的标准化接入层** — 以 MCP Client 调用 Prometheus、只读数据库等 Tool，供 6.5 Orchestrator（LLM function calling）执行  
> 参考业界：Anthropic Model Context Protocol (MCP)、OpenAI Function Calling、Prometheus HTTP API
...
在本项目中，6.5 Tool Executor **不直接** 调 Prometheus/SQL，而是 **只调本模块的 `McpClient`**。工具是否调用、参数如何填写，由 LLM function calling 决定（不再使用关键词 resolver）。
...
| 阶段 | 方案 |
|------|------|
| MVP | LLM function calling → McpClient.call_tool（白名单参数） |
...
| Tool 选路 | LLM function calling（已落地） | 动态 list_tools + 审批 | — |
